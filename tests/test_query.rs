...
fn query_to_json(args: &[&str], tmp_name: &str) -> Value {
    ...
    serde_json::from_str(&content)
        .unwrap_or_else(|e| panic!("Invalid JSON in {tmp_path}: {e}\nContent: {content}"))
}
...
fn query_inputs() {
    ...
    assert!(
        ...
        "Expected 'my_count' key in output, got: {result}"
    );
}
...
fn assert_node_inputs(node: &str, inputs: &Value) {
    let arr = inputs
        .as_array()
        .unwrap_or_else(|| panic!("Node '{node}': expected array, got {inputs}"));
...
fn assert_input_item(node: &str, item: &Value) {
    let obj = item
        .as_object()
        .unwrap_or_else(|| panic!("Node '{node}': input should be object, got {item}"));

    let handle = obj
        .get("handle")
        .and_then(|h| h.as_str())
        .unwrap_or_else(|| panic!("Node '{node}': missing 'handle' field in {item}"));

    match handle {
        "in" => {
            assert!(
                obj.get("value").is_some_and(|v| v.is_number()),
                "Node '{node}': handle 'in' should have number value, got {item}"
            );
        }
        "my_count" => {
            assert!(
                obj.get("value").is_none(),
                "Node '{node}': handle 'my_count' should have no value, got {item}"
            );
        }
        _ => panic!("Node '{node}': unexpected handle '{handle}'"),
    }
}
