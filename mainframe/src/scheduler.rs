                    ...
                    Ok(SchedulerCommand::SpawnExecutorTimeout {
                        ...
                        subscribers.retain(|_, sender| {
                            if let Err(e) = sender.send(ReceiveMessage::ExecutorTimeout {
                                ...
                                warn!("Scheduler send executor timeout to subscriber failed, removing: {e}");
                                false
                            } else {
                                true
                            }
                                ...
                                ReceiveMessage::ExecutorReady {
                                    ...
                                    subscribers.retain(|_, sender| {
                                        if let Err(e) = sender.send(ReceiveMessage::ExecutorReady {
                                            ...
                                            warn!("Scheduler send executor ready to subscriber failed, removing: {e}");
                                            false
                                        } else {
                                            true
                                        }
...
mod tests {
    ...
    fn test_output_options() {
        ...
        assert!(op.target.is_some_and(|t| {
            t.to_node.is_some_and(|mut inputs| {
                inputs.pop().is_some_and(|node_input| {
                    node_input.eq(&ToNodeInput {
                        node_id: "node1".to_string().into(),
                        input_handle: "input1".to_string().into(),
                    })
                })
            })
        }));
