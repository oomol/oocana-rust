...
use super::{
    FlowFinished, FlowNodesWillRun, FlowStarted, ReporterMessage, ReporterTx, SubflowBlockFinished,
    SubflowBlockOutput, SubflowBlockStarted,
};
...
impl FlowReporterTx {
    ...
    pub fn started(&self, inputs: &Option<BlockInputs>) {
        match self.is_block {
            true => self
                .tx
                .send(ReporterMessage::SubflowBlockStarted(SubflowBlockStarted {
            ...
            false => self.tx.send(ReporterMessage::FlowStarted(FlowStarted {
                session_id: &self.tx.session_id,
                job_id: &self.job_id,
                flow_path: &self.path,
                stacks: &self.stacks.vec(),
    ...
    pub fn will_run_nodes(&self, start: &Vec<String>, mid: &Vec<String>, end: &Vec<String>) {
        match self.is_block {
            ...
            false => self
                .tx
                .send(ReporterMessage::FlowNodesWillRun(FlowNodesWillRun {
                    session_id: &self.tx.session_id,
                    job_id: &self.job_id,
                    flow_path: &self.path,
                    stacks: &self.stacks.vec(),
    ...
    pub fn done(&self, error: &Option<String>) {
        match self.is_block {
            ...
            false => self.tx.send(ReporterMessage::FlowFinished(FlowFinished {
                session_id: &self.tx.session_id,
                job_id: &self.job_id,
                flow_path: &self.path,
                stacks: &self.stacks.vec(),
    ...
    pub fn result(&self, result: Arc<OutputValue>, handle: &str) {
        if self.is_block {
            self.tx
                .send(ReporterMessage::SubflowBlockOutput(SubflowBlockOutput {
                    session_id: &self.tx.session_id,
                    job_id: &self.job_id,
                    block_path: &self.path,
                    stacks: &self.stacks.vec(),
